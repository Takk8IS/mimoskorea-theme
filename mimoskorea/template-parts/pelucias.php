<?php
/**
 * Template Part: Pelúcias Section
 * 
 * Seção "Pelúcias" - Produtos WooCommerce mais recentes
 * da categoria "pelucias" organizados por data de publicação
 * 
...
// Obter os 10 produtos mais recentes da categoria "pelucias"
$args = array(
    'post_type' => 'product',
    'posts_per_page' => 10,
    'post_status' => 'publish',
...
$pelucias_query = new WP_Query($args);

// Se não houver produtos, não exibir a seção
if (!$pelucias_query->have_posts()) {
    // Debug: mostrar informações sobre a query
    echo "<!-- DEBUG: Nenhum produto encontrado na categoria 'pelucias' -->";
    echo "<!-- Query args: " . print_r($args, true) . " -->";
    echo "<!-- Found posts: " . $pelucias_query->found_posts . " -->";
    wp_reset_postdata();
    return;
}

// Debug: mostrar que encontrou produtos
echo "<!-- DEBUG: Encontrados " . $pelucias_query->found_posts . " produtos na categoria 'pelucias' -->";
?>

<section class="pelucias-section full-width-section">
    <div class="content-container">
        <!-- Título da seção -->
        <h2 class="pelucias-title">Pelúcias</h2>
        
        <!-- Grid de produtos - 2 linhas × 5 colunas -->
        <div class="pelucias-grid" data-list-name="Pelúcias">
            <?php while ($pelucias_query->have_posts()) : 
                $pelucias_query->the_post();
            ...
            <article class="pelucias-item" data-product-id="<?php echo esc_attr($product_id); ?>" data-product-sku="<?php echo esc_attr($product_sku); ?>" data-product-name="<?php echo esc_attr($product_name); ?>" data-product-price="<?php echo esc_attr($product_price_value); ?>" data-product-currency="<?php echo esc_attr($product_currency); ?>" data-product-categories="<?php echo esc_attr($product_categories_value); ?>" onclick="window.location.href='<?php echo esc_url($product_link); ?>'">
                ...
                <div class="pelucias-item-info">
                    <!-- Nome do produto -->
                    <h3 class="pelucias-item-name">
                        <?php echo esc_html($product_name); ?>
                    </h3>
                    
                    <!-- Descrição do produto -->
                    <?php if (!empty($product_description)) : ?>
                    <p class="pelucias-item-description">
                        <?php echo wp_kses_post($product_description); ?>
                    </p>
                    <?php endif; ?>
                    
                    <!-- Preço do produto -->
                    <div class="pelucias-item-price">
